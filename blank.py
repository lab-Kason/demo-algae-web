...
if page != "monitor":
    ...
    selected_tank = st.selectbox(
        label="",
        options=valid_tanks,
        index=valid_tanks.index(current_tank) if current_tank in valid_tanks else 0,
...
auto_refresh = st.sidebar.checkbox(t("auto_refresh_label"), value=True, key="auto_refresh_checkbox")
...
selected_tank = st.selectbox(
    t("switch_tank"),
    options=valid_tanks,
    index=valid_tanks.index(tank_id) if tank_id in valid_tanks else 0,
    key="monitor_tank_selector"
)
if selected_tank != tank_id:
    st.session_state.pause_autorefresh = True
    st.query_params.tank = selected_tank
    st.rerun()